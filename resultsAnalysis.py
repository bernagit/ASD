...
MAX_COLUMNS = 20  # Numero massimo di colonne per eseguire il solver
...
def analyze_benchmarks():
    ...
    for filename in benchmark_files:
        ...
        if columns_after_dz <= MAX_COLUMNS:
            ...
            termination_status_dz = "TERMINATED" if elapsed_time_dz <= TIME_LIMIT else "TIME LIMIT"
        else:
        ...
        solver_dz_dd = Solver(instance_matrix, filename, opt_dz_dd)
        ...
        if columns_after_dz_dd <= MAX_COLUMNS:
            ...
            tracemalloc.stop()  # Ferma il monitoraggio della memoria
            termination_status_dz_dd = "TERMINATED" if elapsed_time_dz_dd <= TIME_LIMIT else "TIME LIMIT"
        else:
